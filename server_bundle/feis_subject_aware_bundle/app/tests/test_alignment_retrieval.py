...
class _DummyDataset:
    def __init__(self, protocol: str):
        ...
        self._targets = {
            "01:a": {"speech_embedding": np.asarray([1.0, 0.0], dtype=np.float32)},
            "01:b": {"speech_embedding": np.asarray([0.0, 1.0], dtype=np.float32)},
            "02:a": {"speech_embedding": np.asarray([0.9, 0.1], dtype=np.float32)},
            "03:a": {"speech_embedding": np.asarray([0.85, 0.15], dtype=np.float32)},
        }
        self._waveforms = {
    ...
    def get_template_target(self, template_id: str):
        return dict(self._targets[template_id])
...
class AlignmentRetrievalTests(unittest.TestCase):
    ...
    def test_exact_and_label_rank_metrics_diverge(self):
        bank = RetrievalBank(
            ...
            embeddings=np.asarray([[1.0, 0.0], [0.0, 1.0], [0.9, 0.1]], dtype=np.float32),
            waveforms=np.stack([_tone(220.0), _tone(440.0), _tone(260.0)], axis=0),
        )
        predicted = np.asarray([[1.0, 0.0], [0.95, 0.05]], dtype=np.float32)
        ...
        self.assertAlmostEqual(exact_eval["metrics"]["retrieval_top3_exact"], 1.0)
        ...
        self.assertAlmostEqual(label_metrics["retrieval_top3_label"], 1.0)
    ...
    def test_waveform_nta_exact_and_label(self):
        ...
        label_bank = RetrievalBank(
            ...
            embeddings=exact_bank.embeddings,
