...
模型主体采用 sensor-aware EEG encoder、latent token former 与 hierarchical grouped RVQ。RVQ 不再被视为一个完全共享的黑盒 codebook，而是被划分为 base auditory、content、prosody、voice 和 residual nuisance 五组。alignment heads 只读取与任务相关的 code groups，q7 residual code 只参与低权重重构，不进入任何 attribute 或 retrieval head。
...
| --- | --- | --- |
| BrainOmni / LUNA | EEG/MEG foundation backbone、topology-aware 或 topology-agnostic sensor modeling | sensor-aware temporal encoder、latent query aggregation、channel mask |
...
训练时 q7 采用 group dropout。dropout 的作用不是 regularization 的附属细节，而是防止 full decoder 将 q7 变成主要重构通道。评估中单独报告 q7 的 codebook usage、perplexity、dropout ablation 和 dataset predictability。若 q7 对 dataset_id 的预测过强，该现象被解释为 nuisance absorption，而不是 token 表征质量提高。
...
## 10. Future code interface

The current `src/eeg_voice_model` v0.1 already contains useful components: sensor embedding, temporal encoder, RVQ, token metrics, phoneme head and contrastive retrieval head. V1 implementation can keep these ideas while replacing the interface with grouped-token semantics.
...
| q7 nuisance behavior | q7 usage、q7 perplexity、dataset predictability、dropout ablation | residual 是否变成 dataset shortcut |
...
V1 的代码重构可以直接替换 `src/eeg_voice_model` 的现有接口，但文档阶段不改代码。后续实现中，v0.1 的 `SensorEmbedding`、temporal encoder、RVQ、token metrics、phoneme head 和 contrastive head 可以作为实现素材；需要改变的是 RVQ grouping、head routing、q7 policy、mode adapter、retrieval queue 和 batch schema。
...
q7 metrics include usage / perplexity / dropout ablation / dataset predictability
```
