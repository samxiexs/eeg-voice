...
V1 模型第一版代码已经落地。当前实现不再沿用 v0.1 的 `TokenCentricEEGVoiceModelV01` 主入口，而是切换为 `EEGVoiceTokenV1`。模型目标与 `docs/model_v1_design_0518.md` 保持一致：
...
其中 `datasets.py` 和 `audio_features.py` 仍是 ds006104 / ds005345 早期适配逻辑，后续应由 V1 dataset registry 取代。
