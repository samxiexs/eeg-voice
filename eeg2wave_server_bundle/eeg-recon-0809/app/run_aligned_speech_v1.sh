...
ALIGNED_FULL_OUTPUT="${ALIGNED_FULL_OUTPUT:-$ALIGNED_OUTPUT}"
ALIGNED_M0_REPORT="${ALIGNED_M0_REPORT:-$ALIGNED_OUTPUT/m0_report.json}"
ALIGNED_ALIGN_LR="${ALIGNED_ALIGN_LR:-0.0003}"
ALIGNED_FINETUNE_LR="${ALIGNED_FINETUNE_LR:-0.0003}"
read -r -a seeds <<< "${ALIGNED_SEEDS:-31 47 73}"
for seed in "${seeds[@]}"; do
  if [[ ! "$seed" =~ ^[1-9][0-9]*$ ]]; then
    echo "Seed must be a positive integer: $seed" >&2; exit 2
  fi
done
EEG_GATE_ARGS=(--audio-review "$ALIGNED_OUTPUT/audio_review.json" --m0-report "$ALIGNED_M0_REPORT")
if [[ "${ALIGNED_OBJECTIVE_ONLY:-0}" == "1" ]]; then
  EEG_GATE_ARGS=(--objective-only --m0-report "$ALIGNED_M0_REPORT")
fi
stage="${1:-readiness}"
...
case "$stage" in
  ...
  m0)
    ...
    if [[ -f "$ALIGNED_OUTPUT/m0_finetune/best_m0_checkpoint.pt" ]]; then m0_checkpoint="$ALIGNED_OUTPUT/m0_finetune/best_m0_checkpoint.pt"; fi
  ...
  full)
    for seed in "${seeds[@]}"; do
      ...
      for lag in 0 100 200 300 400; do
        trial_root="$ALIGNED_FULL_OUTPUT/seed-$seed/lag-$lag"
      ...
      selection="$ALIGNED_FULL_OUTPUT/seed-$seed/selection.json"
      ...
      run evaluate --role test --device "$ALIGNED_DEVICE" --checkpoint "$selected_checkpoint" --selection "$selection" \
        --output "$ALIGNED_FULL_OUTPUT/seed-$seed/test_evaluation.json"
      run export --kind eeg --role test --device "$ALIGNED_DEVICE" --checkpoint "$selected_checkpoint" --selection "$selection" \
        --output "$ALIGNED_FULL_OUTPUT/seed-$seed/eeg_listening"
    done
    ;;
