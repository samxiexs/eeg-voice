...
> 明日汇报用简版｜当前阶段：模型、音频端可行性门槛与一键训练流程已搭建；实验尚在运行，因此不汇报重建效果。

## 0. 一句话讲清楚

我们希望训练一个**推理时只输入 EEG** 的生成模型：先把多通道 EEG 压缩成 50 个连续“声学条件 token”，再预测 EnCodec 的离散音频码，最后解码成波形。
...
最终生成器不接收 label、受试者 ID 或数据集 ID。当前无外部语音版本先从头训练一个短程 0715-style 音频 bootstrap，再只迁移其中**名称和形状兼容、且不属于 label/dataset/subject 模块**的参数，随后用连续 HuBERT condition 适配。最终推理链路仍然不依赖固定 label 词表。
...
```mermaid
flowchart LR
    A["本轮从头训练<br/>0715-style bootstrap"] --> B["迁移兼容的共享参数<br/>不复制显式 label/dataset/subject 模块"]
    B --> C["冻结 HuBERT<br/>12 epoch 连续条件适配"]
    C --> D["Validation audio-condition oracle"]
    D -->|"正确 condition 显著优于 shuffled"| E["冻结音频 checkpoint<br/>记录 checkpoint/gate SHA"]
    D -->|"未通过"| X["停止，不训练 EEG"]
    E --> F["EEG 自监督预训练"]
    F --> G["EEG→HuBERT condition→语音训练"]
```

这个顺序先回答一个必要问题：**如果给音频生成器正确的 HuBERT condition，它能不能还原相应音频？**如果这一步都失败，继续训练 EEG encoder 没有意义，因为 EEG 只能提供更困难、更有噪声的 condition。

这里有三个需要准确表达的边界：

- 不读取已有 `combined_0715_v1` 模型 checkpoint；bootstrap 在本次 0722 输出目录内重新训练。
- 会复用已经完成的 EnCodec code cache。它是预处理数据，不是旧模型结果。
- “不复制 label 参数”不等于这些共享参数在统计意义上从未受 label 监督影响。bootstrap 训练时共享层仍可能接收到 label 相关梯度；因此它只作为优化初始化，真正需要验证的是后续连续 HuBERT condition 能否控制输出，并优于 shuffled condition。
...
注意，当前并不是更新 HuBERT 的 95M 参数。更准确的说法是：**冻结 HuBERT 作为声学教师，训练其后的 768→192 condition projector、Audio Condition Encoder 与 MaskGIT。**这样既利用 HuBERT 的连续语音表征，又避免小数据把预训练语音模型本身训练坏。
...
HuBERT 不负责生成，它只是提供连续声学 token，并告诉 EEG encoder：“与这段声音对应的表示应该靠近哪里”。HuBERT 全程冻结；实际被微调的是后面的 condition projector、Audio Condition Encoder 和 MaskGIT。教师冻结可减少小数据下把预训练声学空间一起训练坏的风险。
...
0722 主生成器没有 label embedding，也没有 label probability；改变 label 文件不会改变推理输出。短程 bootstrap 中用过 label 监督，但显式 label/dataset/subject 参数不会迁移到 0722 主模型。
...
2. **Fresh 0715-style bootstrap**：在本次 0722 输出目录从头训练 20 epoch，不读取已有 0715 checkpoint。当前模型结构检查可迁移 186 个同名同形状的非显式-label参数，并排除名称含 `label/dataset/subject/text_projector` 的参数。
3. **HuBERT-condition audio adaptation**：以 `1×10⁻⁴` 的共享初始化学习率训练 12 epoch。训练时 65%–95% EnCodec token 被 mask，且 50% batch 使用 full mask，迫使生成器真正利用连续 HuBERT condition；condition dropout 为 10%，避免对教师表示形成脆弱依赖。
4. **Audio-condition-oracle gate**：在 FEIS 与 KaraOne 的全部 validation 唯一音频上解码，不读取 locked test。正确 condition 与同数据集、无自环的 shuffled condition 分别生成完整波形；只有绝对质量和 condition-specific 增益都通过，才绑定 checkpoint SHA、gate SHA 和 lineage，并冻结复用。失败则不启动 EEG 训练。
5. **EEG 自监督预训练**：三个数据集共同训练 masked patch reconstruction、通道视图一致性和 MoE。
6. **配对 EEG–audio 训练**：音频 decoder 全程冻结。KaraOne 提供完整生成监督，FEIS 提供弱对齐，ds004306 只继续自监督。
7. **Validation 选 checkpoint**：按受试者宏平均的 envelope、调制谱、log-mel 和 retrieval 选择，不用 label accuracy 选模型。
8. **最后才访问 locked test**：通过 validation gate 后做一次正式评估。

本轮首跑采用 `20 bootstrap / 12 audio adaptation / 15 EEG pretrain / 25 paired EEG epochs`。配置文件默认值是 `20 / 12 / 20 / 30`，本轮为了先验证完整链路，用命令行缩短 EEG 两阶段。它仍是无外部语音的 proof-of-concept；真正开放词汇训练将在未来使用公开中英文语音重新训练独立 audio prior。

### 7.1 Audio-condition-oracle 如何判定“音频端可用”

`audio-condition oracle` 不是 EEG 重建结果，也不是最终性能。它是音频生成器的上界单元测试：输入来自同一条真实音频的 HuBERT condition，检查生成器是否能重建该音频的结构。

| Validation gate | 通过阈值 | 为什么需要 |
|---|---:|---|
| Median ±250 ms envelope correlation | ≥ 0.20 | 至少恢复粗略能量时间结构 |
| Median modulation correlation | ≥ 0.15 | 恢复能量变化节律 |
| Median log-mel MAE | ≤ 12 dB | 频谱形状不能偏差过大 |
| Median q0 code accuracy | ≥ 0.03 | 最粗粒度 EnCodec code 必须高于近随机水平 |
| Envelope gain over shuffled | ≥ 0.02 | 正确 condition 必须比错误 condition 更好 |
| Log-mel gain over shuffled | ≥ 0.10 dB | 排除只生成平均模板的可能 |

绝对指标回答“声音是否大致重建”；相对 shuffled 指标回答“生成结果是否真的由这条 condition 决定”。两者缺一不可。阈值是本轮工程 gate，并非已经取得的实验结果。

### 7.2 哪些内容会重跑，哪些内容会复用

| 内容 | 第一次完整运行 | Gate 通过后的后续运行 |
|---|---|---|
| EnCodec 项目音频 cache | 复用现有预处理结果 | 继续复用 |
| HuBERT teacher cache | 构建或重建一次 | 输入和配置不变时复用 |
| 0715-style bootstrap | 在 0722 目录从头训练 | 不再运行 |
| HuBERT-condition audio adaptation | 训练 12 epoch | 复用冻结 checkpoint |
| Audio oracle audit | 完整解码并判定 | 校验 checkpoint/gate/lineage 哈希 |
| EEG pretrain 与 paired training | 训练 | 可按实验需要重训 |
...
> 我们现在做的是一个推理时 label-free 的 EEG 到语音生成模型。三个数据集共有 7,143 个 trial，但只有 KaraOne 的 1,913 个 trial 有同 trial 独立音频，所以它负责精确生成监督；FEIS 只做弱对齐，ds004306 主要做 EEG 自监督。为了避免音频端没有学会就直接训练 EEG，本轮先在 0722 目录从头训练一个短程 0715-style bootstrap，只迁移非显式-label的兼容参数，再冻结 HuBERT，用连续 HuBERT condition 适配音频生成器。随后做 audio-condition oracle：只有正确 condition 生成的波形在能量、调制谱和 log-mel 上达到门槛，并显著优于 shuffled condition，才冻结音频端并开始 EEG 训练。EEG 端将 14 通道切成 patch，经 MLP、Adapter-MoE 和 50 个 learned queries 得到 50×192 condition，再用 CLIP 式全局与局部对齐学习接近音频 token，MaskGIT 最后预测 EnCodec code 并解码波形。推理时只输入 EEG，不输入 label、受试者或数据集 ID。当前实验先验证这条链路，正式开放词汇结论仍需要公开语音、全通道和留一词实验。

---

## 11. 本轮完整运行命令

第一次从头运行（不下载外部语音、不复用旧 0715 模型 checkpoint）：
...
RETRAIN_PRETRAIN=1 \
REBUILD_TEACHERS=1 \
BOOTSTRAP_AUDIO_EPOCHS=20 \
AUDIO_EPOCHS=12 \
PRETRAIN_EPOCHS=15 \
EEG_EPOCHS=25 \
DEVICE=mps \
bash run_open_vocab_0722_full.sh
```

如果 oracle gate 已通过并已生成冻结清单，后续只重训 EEG：
