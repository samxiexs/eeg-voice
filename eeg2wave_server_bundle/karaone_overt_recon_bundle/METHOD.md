...
> **当前默认流水线（2026 重做，可切换）**：
> **EEG → 编码器(可选通道MoE) → 声学解码头(回归|扩散) → 声学目标(mel|EnCodec) → 声码器(Griffin-Lim|EnCodec) → 波形**。
> 默认 = `mel + 回归 + DTW对齐 + (可选)GAN + Griffin-Lim`，对齐论文（NeuroTalk/Park/FESDE）的
> "mel + 对齐 + 声码器"配方；EnCodec-latent、扩散头、通道MoE 全部保留为开关。
> 完整设计与开关见 [DIFFUSION_PLAN.md / 重做方案](DIFFUSION_PLAN.md) 顶部及 §4。模型**完全不使用被试 ID**。
>
> 下文 §1–§3 描述的 EnCodec-latent + cosine 回归是**早期/可选**路径（实测会塌缩，见 §4）；
> §4 起是为摆脱塌缩而做的生成式/对齐重做。
