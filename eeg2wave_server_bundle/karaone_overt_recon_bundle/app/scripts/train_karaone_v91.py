...
from src.karaone_v91.data import (
    KaraOneV91ClusterBalancedBatchSampler,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train KaraOne v9.1 clustered Channel-MoE semantic-flow stages.")
...
def main() -> None:
    ...
    train_ds = KaraOneV91ClusteredDataset(
        root,
        targets,
        train_split,
    ...
    val_ds = KaraOneV91ClusteredDataset(root, targets, "subject_val", cluster_bank=cluster_bank, stages=stages, subject_val=subject_val, subject_test=subject_test, eeg_len=eeg_len)
    test_ds = KaraOneV91ClusteredDataset(root, targets, "subject_test", cluster_bank=cluster_bank, stages=stages, subject_val=subject_val, subject_test=subject_test, eeg_len=eeg_len)
    model = KaraOneV91ClusteredChannelMoEFlow(make_model_config(cfg, targets, train_ds, eeg_len=eeg_len)).to(device)
    if args.checkpoint:
    ...
    suffix = args.run_suffix or f"v91_{phase}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    stage_tag = "_".join(stages)
    out_root = resolve_bundle_path(cfg["output"]["root"], BUNDLE_DIR)
    out_dir = ensure_dir(out_root / f"{cfg['output'].get('prefix', 'karaone_v91')}_{phase}_{stage_tag}_{suffix}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        iterator = progress(loader, total=len(loader), desc=f"v9.1 {phase} epoch {epoch}")
        for step, batch in enumerate(iterator, start=1):
            ...
            if phase == "pretrain":
                ...
                losses = compute_v91_pretrain_losses(
                    out,
                    batch,
                ...
                losses = compute_v91_transport_losses(
                    out,
                    batch,
                ...
                losses = compute_v91_alignment_losses(
                    out,
                    batch,
        ...
        val_outputs = collect_v91_outputs(model, val_ds, device=device, batch_size=batch_size)
        test_outputs = collect_v91_outputs(model, test_ds, device=device, batch_size=batch_size)
        val_metrics = compute_v91_metrics(val_outputs, train_bank=train_bank, prefix="subject_val")
        test_metrics = compute_v91_metrics(test_outputs, train_bank=train_bank, prefix="subject_test")
...
def make_loader(dataset: KaraOneV91ClusteredDataset, *, batch_size: int, cfg: dict) -> DataLoader:
    use_sampler = bool(cfg.get("clusters", {}).get("cluster_balanced_sampler", True)) and dataset.cluster_bank.available
    if use_sampler:
        sampler = KaraOneV91ClusterBalancedBatchSampler(dataset, batch_size=batch_size, seed=int(cfg.get("train", {}).get("seed", 7)))
...
def print_run_header(
    *,
    ...
    out_dir: Path,
    root: Path,
    cluster_bank: KaraOneV91ClusterBank,
    train_ds: KaraOneV91ClusteredDataset,
    val_ds: KaraOneV91ClusteredDataset,
    test_ds: KaraOneV91ClusteredDataset,
...
def dataset_target_bank(dataset: KaraOneV91ClusteredDataset) -> dict[str, Any]:
    rows = [dataset[i] for i in range(len(dataset))]
    return outputs_to_v91_bank(
        {
...
def selection_score(metrics: dict[str, Any]) -> float:
    ...
    ) + 0.25 * float(metrics.get("subject_val_same_label_cross_subject_gain", 0.0)) + 0.1 * float(
        metrics.get("subject_val_cluster_label_top3", 0.0)
    ) - 0.05 * float(metrics.get("subject_val_subject_leakage_acc", 0.0))
...
def save_checkpoint(model: torch.nn.Module, path: Path, *, cfg: dict, epoch: int, metrics: dict[str, Any]) -> None:
    ...
    torch.save(
        {"model": model.state_dict(), "config": cfg, "epoch": epoch, "metrics": metrics, "model_kind": "karaone_v91_clustered_channel_moe_flow"},
        path,
    )
