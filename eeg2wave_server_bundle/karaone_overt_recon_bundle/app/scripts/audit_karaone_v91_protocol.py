...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Audit KaraOne v9.1 clustered subject-holdout protocol.")
    parser.add_argument("--config", default=str(BUNDLE_DIR / "configs" / "karaone_v91.yaml"))
    parser.add_argument("--cluster-audit", default=None)
    parser.add_argument("--out", default="../artifacts/audio_targets/karaone_v91_protocol_audit.json")
...
def main() -> None:
    ...
    if audit_path.exists():
        ...
        checks.append(
            check(
                ...
                all(bool(cluster_audit.get("cluster_coverage", {}).get(name)) for name in ("eeg_cluster_id", "speech_cluster_id", "cross_modal_cluster_id")),
    ...
    checks.append(
        {
            "name": "v9.1 waveform gate declared",
            "status": "pass",
            "detail": {
                "rule": "transport/waveform outputs are diagnostic until semantic/prosody subject_val gate passes and signs hold on subject_test",
    ...
    payload = {
        "audit_kind": "karaone_v91_protocol_audit",
        "status": status,
